...
async function request(url, options) { let response; try { response = await fetch(url, options); } catch (_) { throw Error("无法连接到 Heka 本地服务。请运行 python3 server.py，并通过 http://127.0.0.1:8787 打开。 "); } let body; try { body = await response.json(); } catch (_) { throw Error("本地服务返回了异常内容。请重启 python3 server.py 后重试。"); } if (!response.ok) throw Error(body.error || "请求失败。"); return body; }
